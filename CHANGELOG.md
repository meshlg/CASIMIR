...
- Quantum bytes from ANU QRNG are now XOR-mixed with local CSPRNG (`crypto.getRandomValues`) output, so generated passwords remain cryptographically strong even if the remote API is compromised or intercepted.
- Recommendation text and details are now HTML-escaped before rendering, preventing markup injection from analyzed password fragments.
- Added `Add-Padding: true` header to HIBP k-Anonymity requests for better privacy (response-size analysis resistance).

### Fixed

- `@updateURL`/`@downloadURL` now point to the Greasy Fork `.user.js`/`.meta.js` endpoints instead of the HTML script page, so auto-updates work.
- Recommendation `detail` texts are now actually displayed under each recommendation (previously computed but never shown).
- Entropy-penalty and GPU crack-time recommendations are now localized (previously always shown in Russian).
- Removed a duplicate branch in `formatCrackTime`; the 100–1000 year range now formats consistently.
- Strength meter now stays in sync with the test-password input (previously kept showing the old generated password).
- Quantum API cooldown raised from 5 s to 60 s to match the ANU QRNG free-tier rate limit (1 request/minute).

### Changed

- Script file renamed to `CASIMIR.user.js` so userscript managers recognize it on direct install.
- Added `@noframes` and `@run-at document-idle` metadata (iframe guard now enforced by the manager, not only in code).

## [1.0.1.1814] - 2026-02-24

### Fixed

- Fixed tooltip text wrapping: each character no longer appears on a separate line.
- Fixed language toggle button (EN/РУ) text wrapping: button now displays text in a single line.
- Fixed floating toggle button icon alignment: icon is now properly centered.

### Changed

- Removed decorative glow effects from UI corners (both inside panel and on root wrapper).

## [1.0.1.1159] - 2026-02-24
